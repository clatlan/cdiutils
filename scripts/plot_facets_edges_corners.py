...
if __name__ == '__main__':
    ...
    if args["files"] is None:
        file_template = "results/facet_analysis/S{}/S{}_planes_iso0.65.dat"
    ...
    ax.errorbar(scan_digits, surface_strain, surface_strain_std,
                label="Surface strain", marker="o", capsize=5.0, capthick=1.5)
    ...
    ax.set_xlim(177, 185.7)

    ax.set_xticklabels(xticks_labels, fontsize=9)
    ax.set_xlabel("Course of experiment (scan \#)", fontsize=16)
    ax.set_ylabel("Strain (\%)", fontsize=16)
    fig.legend(loc="upper center", bbox_to_anchor=(0.5, 0.94), fontsize=12,
               ncol=len(ax.lines))
    fig.suptitle("Surface, edge and corner strain evolutions", fontsize=20)
