...
class SupportProcessor:
        ...
        try:
            ...
            if self.previous_input_parameters[2] != self.input_parameters[2]:
                try:
                    ...
                    print(
                        f"The folder at {self.path_surface}"
                ...
                if not np.array_equal(
                    ...
                    print(
                        "amplitude_threshold or derivative_threshold"
                        "has been changed"
                    )
                    try:
                        shutil.rmtree(self.path_order)
                        print(
                            f"The folder at {self.path_order}"
                            f"has been successfully removed."
                        )
                    except:
    ...
    def def_derivative_value_direction(self) -> None:
        ...
        try:
            ...
            grad_val = np.load(f"{self.path_surface}grad_val.npy")
            ...
            mask = self.amplitude > (self.amplitude_threshold * amplitude_max)
            ...
            grad_dir[mask, :] = -grad_dir[mask, :] / norm[mask, None]

            grad_val[mask] = np.sum(
                (gradient_int[mask] * grad_dir[mask]), axis=-1
            )

            np.save(f"{self.path_surface}gradient.npy", gradient)
            np.save(f"{self.path_surface}grad_dir.npy", grad_dir)
            np.save(f"{self.path_surface}grad_val.npy", grad_val)
        ...
        if self.order_of_derivative == "laplacian":
            try:
                ...
                np.save(f"{self.path_surface}/laplacian.npy", laplacian)
    ...
    def def_surface_candidat(self, grad_val, laplacian) -> None:
        ...
        try:
            surface_voxel_candidat = np.load(
                f"{self.path_order}surface_voxel_candidat.npy"
            )
        except FileNotFoundError:
            ...
            if self.order_of_derivative == "gradient":
                ...
                voxel_indices = np.argwhere(
                    grad_val < (self.derivative_threshold * grad_min)
                )
            ...
            if self.order_of_derivative == "laplacian":
                ...
                voxel_indices = np.argwhere(
                    laplacian < (self.derivative_threshold * lapl_min)
                )
                surface_voxel_candidat = voxel_indices.tolist()
    ...
    def interpolate_voisins(
        self, i, j, k, dir, s, xgrad, ygrad, zgrad, laplacian
    ):
        """
        Args:
            ...
            for b in range(2):
                for c in range(2):
                    ...
                    if self.order_of_derivative == "gradient":
                        ...
                        voisins[a][b][c] = gradxyz / math.sqrt(
                            dx * dx + dy * dy + dz * dz
                        )

                    elif self.order_of_derivative == "laplacian":
                        laplacianxyz = laplacian[
                            int(i + a * ux), int(j + b * uy), int(k + c * uz)
                        ]
    ...
    def interpolation(
        self, i, j, k, dir, xgrad, ygrad, zgrad, laplacian
    ) -> [float, float]:
        """
        Args:
            i,j,k (int) : position of a voxel
        ...
        avant = self.interpolate_voisins(
            i, j, k, dir, 1, xgrad, ygrad, zgrad, laplacian
        )

        arriere = self.interpolate_voisins(
            i, j, k, dir, -1, xgrad, ygrad, zgrad, laplacian
        )
    ...
    def def_p_surface(
        ...
        try:
            pre_surface = np.load(f"{self.path_order}/preprocess_surface.npy")
            surface_direction = np.load(
                f"{self.path_order}/surface_direction.npy"
            )
        except FileNotFoundError:
            ...
            for elem in surface_voxel_candidat:
                ...
                if (
                    ...
                    dir = grad_dir[i][j][k]
                    if self.order_of_derivative == "gradient":
                        fw_grad, bw_grad = self.interpolation(
                            i, j, k, dir, xgrad, ygrad, zgrad, laplacian
                        )
                        condition_met = (
                            grad_val[i][j][k] < fw_grad
                            and grad_val[i][j][k] < bw_grad
                        )
                    elif self.order_of_derivative == "laplacian":
                        fw_lapl, bw_lapl = self.interpolation(
                            i, j, k, dir, xgrad, ygrad, zgrad, laplacian
                        )
                        condition_met = (
                            laplacian[i][j][k] < fw_lapl
            ...
            np.save(f"{self.path_order}/preprocess_surface.npy", pre_surface)
            np.save(
                f"{self.path_order}/surface_direction.npy", surface_direction
            )
        return pre_surface
            ...
            for x, y, z in zip(*np.nonzero(pre_surface)):
                ...
                for offset in offsets:
                    ...
                    if pre_surface[adj_voxel] > 0:
                        ...
                        nb_voisins += 1
                s_dir /= nb_voisins
                smooth_dir[x, y, z] = s_dir / (
                    npnorm(s_dir) if npnorm(s_dir) != 0 else 1
                )
    ...
    def distance_filled_surface(self, a, b, u) -> float:
        ...
        return abs(np.dot(b - a, v_unitaire)) + (N + 1) * npnorm(
            (b - a) - np.dot(b - a, v_unitaire) * v_unitaire
        )
    ...
    def def_filled_slice(
        self, slice_surface, list_coord_contour, slice_direction, mark
    ) -> np.ndarray:
        """
        Args:
        ...
        for pixel0 in list_coord_contour:
            ...
            if max(abs(m2), abs(m3)) > 0:
                ...
                for t in range(2):
                    ...
                    offsets = [
                        (db, dc) for db in [-1, 0, 1] for dc in [-1, 0, 1]
                    ]
                    ...
                    while len(queue) > 0 and (min_dist == -1 or min_dist > 1):
                        ...
                        if 0 <= b < N2 and 0 <= c < N3:
                            ...
                            if min_dist == -1:
                                for offset in offsets:
                                    adj_pixel = tuple(
                                        x + y for x, y in zip(c_pixel, offset)
                                    )
                                    if (
                                        adj_pixel not in visited
                                        and 0 <= adj_pixel[0] < N2
                                        and 0 <= adj_pixel[1] < N3
                                    ):
                                        if (
                                            test(adj_pixel[0], adj_pixel[1])
                                            == t
                                        ):
                    ...
                    if min_dist > 1:
                        for elem in plus_proche_voisins:
                            ub = (elem[0] - b0) / math.sqrt(
                                (b0 - elem[0]) ** 2 + (c0 - elem[1]) ** 2
                            )
                            uc = (elem[1] - c0) / math.sqrt(
                                +((b0 - elem[0]) ** 2) + (c0 - elem[1]) ** 2
                            )

                            sorted_dir = sorted(
                                [(abs(ub), ub, 0), (abs(uc), uc, 1)],
                                reverse=True,
                            )
                            u1 = sorted_dir[0][1]
                            u2 = sorted_dir[1][1]
    ...
    def def_filled_surface(
        self, surface_to_be_filled, smooth_dir
        ...
        for mark in range(3):
            ...
            for a in list_coord_surface:
                ...
                f_slice = self.def_filled_slice(
                    slice_surface, list_coord_contour, slice_direction, mark
                )
    ...
    def def_filled_presurface(self, pre_surface, smooth_dir) -> None:
        ...
        """
        try:
            f_presurface = np.load(f"{self.path_order}/filled_presurface.npy")
        except:
    ...
    def support_calculation(self) -> np.ndarray:
        if self.params["support_path"] is None:
            ...
            if self.params["support_method"] == "isosurface":
                ...
                (xgrad, ygrad, zgrad, grad_dir, grad_val, laplacian) = (
                    self.def_derivative_value_direction()
                )
                surface_voxel_candidat = self.def_surface_candidat(
                    grad_val, laplacian
                )
                pre_surface = self.def_p_surface(
                    xgrad,
                    ygrad,
                    zgrad,
                    grad_dir,
                ...
                if self.raw_process:
                    ...
                    f_presurface = self.def_filled_presurface(
                        pre_surface, smooth_dir
                    )
                    try:
                        p_support = np.load(
                            f"{self.path_order}/processed_support.npy"
                        )
                    except:
            ...
            np.save(
                f"{self.dump_dir}/surface_calculation/surface.npy", surface
            )
