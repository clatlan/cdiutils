...
if IS_INTERACTIVE_AVAILABLE:
    ...
    class Plotter:
        ...
        def init_plot(self):
            ...
            if self.plot == "2D":
                ...
                max_amp = np.max(amp)
                phase_in_support = np.where(
                    amp > 0.05 * max_amp, phase, np.nan
                )
                ...
                self.plot == "sum_contour_slices" and self.data_array.ndim == 3
            ):
                plot_3d_slices(
                    data_array=self.data_array,
                    fontsize=self.fontsize,
        ...
        def on_update_plot(self, change=None):
            ...
            try:
                ...
                # Linear or log colouring
                    ...
                    self.color = color
                x, y, z = verts.T
                self.mesh = ipv.plot_trisurf(
                    x, y, z, triangles=faces, color=color
                )
    ...
    class TabPlotData(widgets.VBox):
        ...
        def init_plot_data_tab(
            ...
            if data_use in ("2D", "1D"):
                ...
                window_support.children[1].observe(
                    support_handler, names="value"
                )
    ...
    error_msg = (
        "Interactive plotting requires ipywidgets, bokeh, "
        "tornado, h5glance. Install the packages to use this "
        "functionality, or use pip install cdiutils[interactive]"
    )
    ...
    def function_place_holder(*args, **kwargs):
        raise ImportError(error_msg)

    Plotter = _ClassPlaceHolder
    ThreeDViewer = _ClassPlaceHolder
    SupportTools = _ClassPlaceHolder

    plot_data = function_place_holder
    plot_2d_image = function_place_holder
    plot_3d_slices = function_place_holder
...
def complex2rgbalin(
    ...
    Args:
        s: The complex valued array to be converted.
        gamma: The gamma correction to apply to the magnitude, by default 1.0.
        smax: The maximum value to clip the magnitude to, by default None.
        smin: The minimum value to clip the magnitude to, by default None.
        percentile: The percentiles used to compute smin and smax, by default
            (None, None).
        alpha: The minimum and maximum values to use for the alpha channel,
            by default (0, 1).
        final_type: The type of the output image, either 'float' or 'uint8', by
            default 'uint8'.
