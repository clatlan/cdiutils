"""
Interactive GUI for phase retrieval using PyNX.

This module provides the PhaseRetrievalGUI class for configuring and
running phase retrieval algorithms in BCDI workflows.
"""
...
if IS_INTERACTIVE_AVAILABLE and IS_PYNX_AVAILABLE:
    ...
    class PhaseRetrievalGUI(widgets.VBox):
        ...
        Attributes:
            ...
            nb_ml: Input field to specify the number of Maximum Likelihood
                (ML) iterations.
            nb_run: Input field to specify the number of phase retrieval runs.
            ...
            calc_llk: Input field to specify the interval for log-likelihood
                calculations.
            zero_mask: Dropdown to specify whether to force mask pixels to zero.
        """
        ...
        def __init__(
            ...
            """
            Initialise the PhaseRetrievalGUI class.

            This method sets up the graphical user interface (GUI) for phase
            retrieval by defining and initialising various widgets.
            ...
            Args:
                pipeline_instance: An instance of BcdiPipeline to handle the
                    analysis of phase retrieval results.
                box_style: The CSS style applied to the widget container.
            ...
            self.iobs = widgets.Dropdown(
                ...
                + [
                    os.path.basename(f)
                    for f in sorted(
                        glob.glob(work_dir + "*.npz"), key=os.path.getmtime
                    )
                ],
                description="Dataset",
                layout=widgets.Layout(width="90%"),
            ...
            self.children = (
                ...
                widgets.HBox(
                    [self.support_threshold, self.support_only_shrink]
                ),
                widgets.HBox(
                    [
        ...
        def pynx_folder_handler(self, change) -> None:
            """
            Handle changes related to the pynx folder.
        ...
        def pynx_psf_handler(self, change) -> None:
            """
            Handle changes related to the psf.
        ...
        def pynx_peak_shape_handler(self, change) -> None:
            """
            Handle changes related to psf the peak shape.
        ...
        def run_pynx_handler(self, change) -> None:
            """
            Handle changes related to the phase retrieval.
        ...
        def init_phase_retrieval_tab(
            ...
            Args:
                ...
                nb_ml: number of maximum-likelihood conjugate gradient to
                    perform after ER
                nb_run: number of times to run the optimisation
            ...
            if run_phase_retrieval and not run_pynx_tools:
                ...
                try:
                    ...
                    raw_cdi = initialise_cdi_operator(
                        iobs=params["iobs"],
                        mask=params["mask"],
                    ...
                    for i in range(1, nb_run + 1):
                        ...
                        if i == 0:
                            cxi_filename = (
                                "{}/pynx_input_operator_{}.cxi".format(
                                    parent_folder,
                        ...
                        # Initialise the free pixels for FLLK
                        ...
                        # Initialise the support with autocorrelation, if no
                        # support given
                        if not support:
            ...
            if run_pynx_tools and not run_phase_retrieval:
                if run_pynx_tools == "analyse_results":
                    ...
                    indices = [
                        options.index(item)
                        for item in params["best_runs_widget"]
                    ]
    ...
    class PhaseRetrievalGUI:
        def __init__(self, *args, **kwargs):
            raise ImportError("The GUI requires PyNX and ipywidgets.")
