...
class ArgumentHandler():
            ...
            if self.script_type == "all":
                self.arguments = {
                    "preprocessing": self._concatenate(
                        self._checked_args(args["preprocessing"],
                                           "preprocessing"),
                        self._checked_args(args["general"], "preprocessing"),
                    ),
                    "pynx": self._concatenate(
                        self._checked_args(args["pynx"], "pynx"),
                        self._checked_args(args["general"], "pynx")
                    ),
                    "postprocessing": self._concatenate(
                        self._checked_args(args["postprocessing"],
                                           "postprocessing"),
                        self._checked_args(args["general"], "postprocessing")
                    )
                }
            else:
                self.arguments = self._checked_args(args, self.script_type)
    ...
    def _checked_args(self, dic, script_type):
        checked_keys = []
        for key in dic.keys():
