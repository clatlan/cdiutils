...
Input: complex amplitude array from a phasing programm
...
scan = args["scan"] # spec scan number

datadir = "results/S" + str(scan) + "/"
...
cxi_file = glob.glob(os.path.dirname(args["modes"]) + "/*.cxi")[0]
original_size = [i for i in silx.io.open(cxi_file)["entry_1/data_1/data"].shape]
print("original size is ", original_size)

# during phasing automatically. Leave it to () if the shape did not change.
binning = tuple(map(int, args["binning"].split(',')))  # binning factor applied during phasing
...
fix_voxel = np.nan if args["is_orthogonalized"] in ["False", "false", "FALSE", "f", 0, "0"] else 0  # voxel size in nm for the interpolation during the geometrical transformation
...
#################################
beamline = args["beamline"]  # name of the beamline, used for data loading and normalization by monitor and orthogonalisation
# supported beamlines: 'ID01', 'SIXS_2018', 'SIXS_2019', 'CRISTAL', 'P10', '34ID'
rocking_angle = args["rocking_angle"]  # "outofplane" or "inplane", does not matter for energy scan

#  "inplane" e.g. phi @ ID01, mu @ SIXS "outofplane" e.g. eta @ ID01
sdd = args["sample_detector_distance"] # 1.26  # sample to detector distance in m
pixel_size = 55e-6  # detector pixel size in m, taking into account an eventual binning during preprocessing

energy = args["energy"]  # x-ray energy in eV, 6eV offset at ID01
beam_direction = np.array([1, 0, 0])  # incident beam along z

outofplane_angle = args["outofplane_angle"] # detector delta ID01, delta SIXS, gamma 34ID
grazing_angle = args["incidence_angle"]  # in degrees, incident angle for in-plane rocking curves (eta ID01, th 34ID, mu SIXS)
inplane_angle = args["inplane_angle"] # detector nu ID01, gamma SIXS, tth 34ID
tilt_angle = args["angle_step"] #0.0078125  # angular step size for rocking angle, eta


#ID01, mu SIXS, does not matter for energy scan
...
flip_reconstruction = False if args["flip"] in ["False", "false", "FALSE", "f", 0, "0"] else True  # True if you want to get the conjugate object

phase_ramp_removal = 'gradient'  # 'gradient'  # 'gradient' or 'upsampling', 'gradient' is much faster
threshold_gradient = 10  # upper threshold of the gradient of the phase, use for ramp removal

xrayutils_ortho = False if args["is_orthogonalized"] in ["False", "false", "FALSE", "f", 0, "0"] else True   # True if the data is already orthogonalized
...
save = True  # True to save amp.npz, phase.npz, strain.npz and vtk filesf
debug =  False if args["debug"] in ["False", "false", "FALSE", "f", 0, "0"] else True # set to True to show all plots for debugging
...
############################################
align_q = True if args["is_orthogonalized"] in ["False", "false", "FALSE", "f", 0, "0"] else False # if True rotates the crystal to align q along one axis of the array
...
############################################
hwidth = 1  # (width-1)/2 of the averaging window for the phase, 0 means no phase averaging
...
# file_path = [datadir+"pynxraw/" + args["modes"]]
file_path = [args["modes"]]
...
if apodize_flag:
    amp, phase = pu.apodize(amp=amp, phase=phase, initial_shape=original_size, window_type=apodize_window,
                            sigma=sigma, mu=mu, alpha=alpha, debugging=debug)
...
if debug:
    phase, _ = pu.unwrap(avg_obj, support_threshold=threshold_unwrap_refraction, debugging=debug)
    gu.multislices_plot(phase, width_z=2*zrange, width_y=2*yrange, width_x=2*xrange,
...
if not xrayutils_ortho:
    ...
    if debug:
        gu.multislices_plot(abs(obj_ortho), width_z=2 * zrange, width_y=2 * yrange, width_x=2 * xrange, sum_frames=False, plot_colorbar=True, vmin=0, vmax=abs(obj_ortho).max(), title='Amp after orthogonalization')
    ...
    try:
        ...
        file_path = glob.glob(os.path.dirname(args["modes"]) + "/QxQzQy*.npz")[0]
...
if correct_refraction or correct_absorption:
    ...
    path_in = pu.get_opticalpath(support=bulk, direction="in", k=kin, debugging=debug)

    # calculate the optical path of the outgoing wavevector: it is not aligned with any orthogonalized axis
    path_out = pu.get_opticalpath(support=bulk, direction="out", k=kout, debugging=debug)
...
if not xrayutils_ortho:
    if align_axis:
        ...
        amp = pu.rotate_crystal(array=amp, axis_to_align=axis_to_align/np.linalg.norm(axis_to_align),
                                reference_axis=myaxis_inplane, debugging=debug)
    ...
    if align_q:
        ...
        amp = pu.rotate_crystal(array=amp, axis_to_align=myaxis,
                                reference_axis=np.array([q[2], q[1], q[0]])/np.linalg.norm(q), debugging=debug)
