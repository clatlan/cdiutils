...
ap.add_argument("--template-data-file", default='data_mpx4_%05d.edf.gz', type=str,
...
ap.add_argument("--mask", default="", type=str,
                help="optional mask")
...
debug = False if args["debug"] in ["False", "false", "FALSE", "f", 0, "0"] else True  # set to True to see plots

binning = [int(i) for i in args["binning"].split(',')]
  # binning that will be used for phasing
# (stacking dimension, detector vertical axis, detector horizontal axis)
##############################
# parameters used in masking #
##############################
flag_interact = False if args["interact"] in ["False", "false", "FALSE", "f", 0, "0"] else True 
...
######################################
beamline = args["beamline"].upper()  # name of the beamline, used for data loading and normalization by monitor
...
rocking_angle = args["rocking_angle"]  # "outofplane" or "inplane" or "energy"
...
###############################
detector = args["detector"]    # "Eiger2M", "Maxipix", "Eiger4M", "Merlin" or "Timepix"
# nb_pixel_y = 1614  # use for the data measured with 1 tile broken on the Eiger2M
x_bragg = 371  # horizontal pixel number of the Bragg peak, can be used for the definition of the ROI
y_bragg = 360  # vertical pixel number of the Bragg peak, can be used for the definition of the ROI
if beamline == "SIXS_2019":
    roi_detector = [0, 515, 0, 515]
elif beamline == "ID01":
    roi_detector = [0, 516, 0, 516] #[0, 516, 0, 516] #[y_bragg - 168, y_bragg + 168, x_bragg - 140, x_bragg + 140]  # [0, 516, x_bragg-179, x_bragg+181]
...
background_file = args["mask"]  # root_folder + 'background.npz'  #
...
################################################################################
use_rawdata = True if args["orthogonalize"] in ["False", "false", "FALSE", "f", 0, "0"] else False # False for using data gridded in laboratory frame/ True for using data in detector frame
ortho_size = False # False = correct size of the data gridded in laboratory frame, True = same shape of raw data
correct_curvature = False  # True to correcture q values for the curvature of Ewald sphere
sdd = args["sample_detector_distance"]  # in m, sample to detector distance in m
energy = args["energy"]  # np.linspace(11100, 10900, num=51)  # x-ray energy in eV
...
offset_inplane = 3.0001  # outer detector angle offset, not important if you use raw data
...
cch1 = 316.39762809648806  # cch1 parameter from xrayutilities 2D detector calibration, detector roi is taken into account below
cch2 = 116.74779768846457  # cch2 parameter from xrayutilities 2D detector calibration, detector roi is taken into account below
detrot = -1.700  # detrot parameter from xrayutilities 2D detector calibration
tiltazimuth = 221.76801794746098 # tiltazimuth parameter from xrayutilities 2D detector calibration
tilt = 2.795342477745362  # tilt parameter from xrayutilities 2D detector calibration
##################################
...
if len(scans) > 1:
    if center_fft not in ['crop_asymmetric_ZYX', 'pad_Z', 'pad_asymmetric_ZYX', 'crop_sym_ZYX']:
        center_fft = 'skip'
...
for scan_nb in range(len(scans)):
    ...
    if setup.beamline == 'P10':
        ...
        # homedir = root_folder + 'detector/2021_01_21_151602_platinum/'
        homedir = root_folder
        detector.datadir = homedir 
        specfile = specfile_name
        # homedir where data files are
    ...
    if not use_rawdata:
        ...
        savedir = args["output_dir"] + "/S" + str(scans[scan_nb]) + "/pynx/"
        ...
        savedir = args["output_dir"] + "/S" + str(scans[scan_nb]) + "/pynxraw/"
        pathlib.Path(savedir).mkdir(parents=True, exist_ok=True)
        ...
        if use_rawdata:
            ...
            data, mask, q_values, frames_logical = \
                pru.grid_bcdi(data=data, mask=mask, scan_number=scans[scan_nb], logfile=logfile, detector=detector,
                              setup=setup, frames_logical=frames_logical, hxrd=hxrd,
                              correct_curvature=correct_curvature, debugging=debug, 
                              follow_bragg=follow_bragg) # changed here MIR
